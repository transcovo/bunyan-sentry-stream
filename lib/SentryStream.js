...
class SentryStream {
  ...
  write(record) {
    ...
    if (err) {
      const extra = _.omit(record, 'err', 'tags', 'user');
      this.client.captureException(this.deserializeError(err), { extra, level, tags, user });
    } else {
      const extra = _.omit(record, 'msg', 'tags', 'user');
      this.client.captureMessage(record.msg, { extra, level, tags, user });
    }
    return (true);
  }
